class Insert2704OnAccounts < ActiveRecord::Migration[8.1]
  ...
  def up
    ...
    a = Account.find_by_code(ACCOUNT_CODE_LONG_TERM_LOAN_SHAREHOLDER)
    a ||= Account.new(code: ACCOUNT_CODE_LONG_TERM_LOAN_SHAREHOLDER)
    a.name = '長期貸付金（株主）'
    a.dc_type = parent.dc_type
    a.account_type = parent.account_type
    a.display_order = 3
    a.parent_id = parent.id
    a.path = parent.path + '/' + ACCOUNT_CODE_LONG_TERM_LOAN_SHAREHOLDER
