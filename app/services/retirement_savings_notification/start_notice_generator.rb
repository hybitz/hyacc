  ...
  class StartNoticeGenerator
    ...
    def execute
      ...
      scope.find_each do |employee|
        due, start_date = notice_due?(employee)
        next unless due
        begin 
          create_notification_and_user_notifications(employee, start_date)
        rescue => e
          HyaccLogger.error("退職金積立開始のお知らせ生成失敗: notification=なし, employee_id=#{employee.id}, error=#{e.message}")
        end
      end
    end
    ...
    def notice_due?(employee)
      ...
      today = Date.current
      due = notify_date.year == today.year && notify_date.month == today.month
      [due, start_date]
    end

    def create_notification_and_user_notifications(employee, start_date)
