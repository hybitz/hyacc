...
class FinancialReturnStatementsControllerTest < ActionController::TestCase
  ...
  def test_15_地代家賃等の内訳書
    sign_in user
    get :index, :commit => true, :finder => rent_finder
    assert_response :success
  ...
  def test_別表5_1_利益積立金額及び資本金等の計算に関する明細書
    sign_in user
    get :index, :commit => true, :finder => surplus_reserve_and_capital_stock_finder
    assert_response :success
  ...
  def test_別表5_2_租税公課の納付状況等に関する明細書
    sign_in user
    get :index, :commit => true, :finder => tax_and_dues_finder
    assert_response :success
  ...
  def test_別表15_交際費等の損金算入に関する明細書
    sign_in user
    get :index, :commit => true, :finder => social_expense_finder
    assert_response :success
