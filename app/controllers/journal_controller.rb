class JournalController < Base::HyaccController
  ...
  view_attribute :finder, :class => JournalFinder, :only => :list

  before_filter :setup_view_attributes, :only => ['list', 'new', 'new_from_copy', 'show', 'show_except_copy_link', 'edit', 'add_detail']
  ...
  # 新規伝票を既存伝票のコピーから登録
  def new_from_copy
    @journal_header = copy_journal( JournalHeader.find( params[:id] ) )
  ...
  def create
    @journal_header = JournalHeader.new(params[:journal_header])
    @journal_header.company_id = current_user.company.id
    @journal_header.slip_type = decide_slip_type
    @journal_header.create_user_id = current_user.id
    @journal_header.update_user_id = current_user.id
    retrieve_details(@journal_header)
    ...
    begin
      @journal_header.transaction do
        ...
        # 仕訳チェック
        validate_journal(@journal_header)
      ...
      # 選択した勘定科目をカウント
      save_input_frequencies(@journal_header)
  ...
  def edit
    @journal_header = JournalHeader.find( params[:id], :include=>:journal_details )
    @detail_no = @journal_header.journal_details.last.detail_no + 1

    # 編集不可能な伝票区分の場合はエラー
    # ただし、エラーメッセージを表示するのではなく一覧画面に遷移させる
    unless can_edit( @journal_header )
  ...
  def update
    ...
    begin
      @journal_header.transaction do
        @journal_header.slip_type = decide_slip_type( old )
        @journal_header.ym = params[:journal_header][:ym]
        @journal_header.day = params[:journal_header][:day]
        @journal_header.remarks = params[:journal_header][:remarks]
        @journal_header.delete_flag_of_receipt_file = params[:journal_header][:delete_flag_of_receipt_file]
        @journal_header.receipt_file = params[:journal_header][:receipt_file]
        @journal_header.update_user_id = current_user.id
        @journal_header.lock_version = params[:journal_header][:lock_version]
        retrieve_details(@journal_header)
...
private
  ...
  def setup_view_attributes
    ...
    @frequencies = InputFrequency.find(
        :all,
        :conditions=>['user_id=? and input_type=?', current_user.id, INPUT_TYPE_JOURNAL_ACCOUNT_ID],
        :order=>'frequency desc',
        :limit=>current_user.account_count_of_frequencies)

    # 部門選択用リスト
    @branches = Branch.scoped_by_deleted(false)
