...
class Auto::TransferJournal::BranchIdSubAccountTest < ActiveSupport::TestCase
  def setup
  ...
  def test_内部取引で部門補助科目はbranch_idを使う
    ...
    head_office_detail = transfer.journal_details.find do |detail|
      detail.branch_id == head_office.id && detail.sub_account_id == @source_branch.id
    end
  ...
  def test_費用配賦で部門補助科目は配賦先branch_idを使う
    ...
    assert_equal [@source_branch.id], credit_details.map(&:sub_account_id)
  end
end
