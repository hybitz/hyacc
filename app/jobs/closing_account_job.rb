class ClosingAccountJob < ActiveJob::Base
    ...
    ActiveRecord::Base.transaction do
      branch_ids.each do |branch_id|
        ...
        pl_data.each do |account, summary|
          summary.each do |sum|
            financial_statements << FinancialStatement.new(:company_id => fiscal_year.company.id, :branch_id=> branch_id,
                                                           :report_type=>1, :ym=> sum[:ym], :account_id => account.id,
  ...
  def pl_monthly(fiscal_year, branch_id)
    ...
    # 最大ノードレベルを算出
    max_node_level = calc_max_node_level(sum)
    sum
  end
