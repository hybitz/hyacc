module PayrollInfo
  class PayrollLogic
    ...
    def get_total_deemed_salary
      ...
      when 2025
        deemed_salary = (deemed_salary/4000).to_i * 4000 if deemed_salary >= 1_900_000 && deemed_salary <= 6_599_999
      when (2026 ..)
        deemed_salary = (deemed_salary/4000).to_i * 4000 if deemed_salary >= 2_200_000 && deemed_salary <= 6_599_999
      end
      deemed_salary
    end
    ...
    def get_deduction
      ...
      when 2026 ..
        ...
        when 0 ... 691_000
          deduction = 740_000
        when 691_000 ... 741_000
          deduction = deemed_salary
        when 741_000 ... 2_191_000
          deduction = 740_000
        when 2_191_000 ... 2_193_000
          deduction = deemed_salary - 1_451_000
        when 2_193_000 ... 2_196_000
          deduction = deemed_salary - 1_453_000
        when 2_196_000 ... 2_200_000
          deduction = deemed_salary - 1_456_000
        when 2_200_000 .. 3_600_000
          deduction = deemed_salary * 0.3 + 80_000
        when 3_600_001 .. 6_600_000
          deduction = deemed_salary * 0.2 + 440_000
        when 6_600_001 .. 8_500_000
    ...
    def get_withholding_tax
      ...
      # 特別減税
      total_tax = [0, total_tax - e.fixed_tax_deduction_amount.to_i].max

      # 住宅借入金控除
      total_tax = [0, total_tax - e.max_mortgage_deduction.to_i].max
    ...
    def get_health_insurance
      ...
      list.each do |p|
        # 健康保険料(預り金)
        p.payroll_journal.journal_details.where(account_id: deposits_received_account.id,
                                                sub_account_id: health_insurance_sub_accounts).each do |d|
          total_expense = total_expense + d.amount
        end
        # 健康保険料(立替金)
        p.payroll_journal.journal_details.where(account_id: advance_money_account.id,
                                                sub_account_id: health_insurance_sub_accounts).each do |d|
          total_expense = total_expense + d.amount
        end
    ...
    def get_employee_pention
      ...
      list.each do |p|
        # 厚生年金保険料(預り金)
        p.payroll_journal.journal_details.where(account_id: deposits_received_account.id,
                                                sub_account_id: welfare_pension_sub_accounts).each do |d|
          total_expense = total_expense + d.amount
        end
        # 厚生年金保険料(立替金)
        p.payroll_journal.journal_details.where(account_id: advance_money_account.id,
    ...
    def get_child_and_childcare_support
      ...
      list.each do |p|
        # 子ども・子育て支援金(預り金)
        p.payroll_journal.journal_details.where(account_id: deposits_received_account.id,
                                                sub_account_id: childcare_sub_accounts).each do |d|
          total_expense = total_expense + d.amount
        end
        # 子ども・子育て支援金(立替金)
        p.payroll_journal.journal_details.where(account_id: advance_money_account.id,
    ...
    def get_employment_insurance
      ...
      list.each do |p|
        # 雇用保険料(預り金)
        p.payroll_journal.journal_details.where(account_id: deposits_received_account.id,
                                                sub_account_id: employment_insurance_sub_accounts).each do |d|
          total_expense = total_expense + d.amount
        end
        # 雇用保険料(立替金)
        p.payroll_journal.journal_details.where(account_id: advance_money_account.id,
    ...
    def get_withholding_taxes(is_bonus = nil)
      ...
      list.each do |p|
        # 賞与
        p.payroll_journal.journal_details.where(account_id: deposits_received_account.id,
                                                sub_account_id: income_tax_sub_accounts,
