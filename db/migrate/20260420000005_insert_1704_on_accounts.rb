class Insert1704OnAccounts < ActiveRecord::Migration[8.1]
  ...
  def up
    ...
    a = Account.find_by_code(ACCOUNT_CODE_SHORT_TERM_LOAN_SHAREHOLDER)
    a ||= Account.new(code: ACCOUNT_CODE_SHORT_TERM_LOAN_SHAREHOLDER)
    a.name = '短期貸付金（株主）'
    a.dc_type = parent.dc_type
    a.account_type = parent.account_type
    a.display_order = 3
    a.parent_id = parent.id
    a.path = parent.path + '/' + ACCOUNT_CODE_SHORT_TERM_LOAN_SHAREHOLDER
    a.journalizable = true
    a.trade_type = TRADE_TYPE_EXTERNAL
    a.is_settlement_report_account = false
    a.sub_account_type = SUB_ACCOUNT_TYPE_SHAREHOLDER
