...
class Mm::SimpleSlipTemplatesControllerTest < ActionController::TestCase
  ...
  def test_登録
    sign_in admin
    post :create, :params => {:simple_slip_template => valid_simple_slip_template_params}, :xhr => true
    assert_response :success
  ...
  def test_更新
    sign_in admin
    patch :update, :params => {:id => simple_slip_template.id, :simple_slip_template => valid_simple_slip_template_params}, :xhr => true
    assert_response :success
    assert_template 'common/reload'
  end
