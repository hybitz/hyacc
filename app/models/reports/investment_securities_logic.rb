module Reports
  class InvestmentSecuritiesLogic < BaseLogic
    ...
    def get_total_shares_and_trading_values
      ...
      sql.append("  sum(case when securities_transaction_type = #{SECURITIES_TRANSACTION_TYPE_BUYING} then shares else -shares end) as shares,")
      sql.append("  sum(case when securities_transaction_type = #{SECURITIES_TRANSACTION_TYPE_BUYING} then trading_value else -trading_value end) as trading_value")
