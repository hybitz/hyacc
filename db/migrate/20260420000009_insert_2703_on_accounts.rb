class Insert2703OnAccounts < ActiveRecord::Migration[8.1]
  include HyaccConst

  def up
    parent = Account.find_by_code(ACCOUNT_CODE_LONG_TERM_LOAN)

    a = Account.find_by_code(ACCOUNT_CODE_LONG_TERM_LOAN_CUSTOMER)
    a ||= Account.new(code: ACCOUNT_CODE_LONG_TERM_LOAN_CUSTOMER)
    a.name = '長期貸付金（取引先）'
    a.dc_type = parent.dc_type
    a.account_type = parent.account_type
    a.display_order = 2
    a.parent_id = parent.id
    a.path = parent.path + '/' + ACCOUNT_CODE_LONG_TERM_LOAN_CUSTOMER
