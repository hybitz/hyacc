class TaxUtils
  ...
  def self.get_social_insurance(base_ym, prefecture_code, base_salary)
    date = Date.strptime("#{base_ym}01", '%Y%m%d')
    si = TaxJp::SocialInsurance.find_by_date_and_prefecture_and_salary(date, prefecture_code, base_salary)
    convert_tax_jp_si(si)
  end

  def self.get_health_insurance(base_ym, prefecture_code, salary)
    date = Date.strptime("#{base_ym}01", '%Y%m%d')
  ...
  def self.get_welfare_pension(base_ym, salary)
    date = Date.strptime("#{base_ym}01", '%Y%m%d')
  ...
  # 標準報酬の基本情報を取得
  def self.get_basic_info(base_ym, prefecture_code, base_salary)
    date = Date.strptime("#{base_ym}01", '%Y%m%d')
    si = TaxJp::SocialInsurance.find_by_date_and_prefecture_and_salary(date, prefecture_code, base_salary)
    convert_tax_jp_si(si)
  end
